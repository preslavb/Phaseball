...
namespace UnityStandardAssets._2D
{
	public class PlatformerCharacter2D : MonoBehaviour
	{
		...
		[SerializeField] private GameObject m_BallPrefabBlueTeam;
		[SerializeField] private GameObject m_BallPrefabRedTeam;
		...
		[SerializeField] private bool bumped;
		...
		private void OnCollisionEnter2D(Collision2D collision)
		{
			...
			{
				collision.gameObject.GetComponent<Rigidbody2D>().velocity = m_Rigidbody2D.velocity;
		...
		public void Move(Vector2 jumpDirection, bool jump)
		{
			...
			else if (hasBallControl && jump && m_BallCooldown <= 0)
			{
				...
				{
					ballInstance = Instantiate(m_BallPrefabBlueTeam);
				}

				else
				{
					ballInstance = Instantiate(m_BallPrefabRedTeam);
				}
			...
			else if (!m_Grounded && jump && !m_Anim.GetBool("Ground") && currentBoost > 0 && currentBoost <= maxBoost && !hasBallControl && canBoost)
			{ 
